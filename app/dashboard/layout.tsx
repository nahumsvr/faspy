...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col xl:flex-row bg-gradient-to-br from-gray-950 via-[#001f33] to-gray-950 bg-[length:200%_200%] animate-gradient">
      ...
      <aside className="hidden xl:flex fixed top-6 left-6 bottom-6 w-64 bg-gray-900/60 backdrop-blur-xl border border-gray-800/80 rounded-3xl p-6 shadow-[0_0_40px_rgba(0,0,0,0.5)] flex-col z-50">
        ...
        <div className="mb-8">
          <div className="inline-flex items-center px-3 py-1.5 rounded-full bg-secondary/15 border border-secondary/20 shadow-inner">
            ...
            <span className="text-xs font-semibold text-secondary font-sans tracking-wide">Simulación Activa</span>
          </div>
        </div>

        {/* Navigation Links */}
        <nav className="flex-1 space-y-2 font-sans flex flex-col pr-1">
          {navItems.map((item) => {
            const isActive = pathname === item.path;
            return (
              <Link
      ...
      {/* Main Content Area */}
      {/* xl:ml-[18rem] = 288px (24px left margin + 256px sidebar + padding). Uses flex-1 so it takes remaining space */}
      <main className="flex-1 w-full xl:ml-[18rem] min-h-[calc(100vh-72px)] xl:min-h-screen">
